...
namespace ShenYangRemoteSystem
{
    public class ModbusHelper
    {
        ...
        public ModbusHelper(UserControl_Page5 uc5) 
        {
            uc = uc5;
        }

        public UserControl_Page5 uc;
        ...
        public bool ConnectPLC(string ip, int port)
        {
            ...
            {
                DisplayRichTextboxContentAndScroll("连接Modbus_TCP失败：" + ex.Message);
                isConnectPLC = false;
            }
            return isConnectPLC;
        }
        ...
        public bool ReadManyCoilStatus(int startAddress, int length, out bool[] value)
        {
            ...
            {
                DisplayRichTextboxContentAndScroll("返回的字节数组的长度范围为 1~2000");
                return false;
            }
            ...
            if (receiveLength == 0)
            {
                DisplayRichTextboxContentAndScroll("解析接收数据非法，接收的实际数据长度不是 读取线圈状态（位）数量");
                return false;
            }
            else
            {
        ...
        public bool WriteSingleCoilStatus(int startAddress, int buffer)
        {
            ...
            {
                DisplayRichTextboxContentAndScroll("Modbus的起始地址必须在0~9999之间");
                return false;
            }
            if (buffer > 1)
            {
                DisplayRichTextboxContentAndScroll("写线圈状态值范围：(0或者1)");
                return false;
            }
        ...
        public bool WriteManyCoilStatus(int startAddress, int[] buffer)
        {
            ...
            {
                DisplayRichTextboxContentAndScroll("写线圈状态范围：(1 至1968 个寄存器)");
                return false;
            }
            for (int i = 0; i < buffer.Length; i++)
            {
                if (buffer[i] > 1)
                {
                    DisplayRichTextboxContentAndScroll("buffer所有数据值必须为0或者1...");
                    return false;
                }
            }
            byte[] addrArray = BitConverter.GetBytes((ushort)startAddress);
        ...
        public bool ReadSingleHoldingRegisterValue<T>(int startAddress, out T value)
        {
            ...
            else
            {
                ...
                DisplayRichTextboxContentAndScroll("读Modbus数据暂不支持其他类型：" + value.GetType());
                return false;
            }
        ...
        public bool ReadManyHoldingRegisterValue(int startAddress, int length, out byte[] value)
        {
            ...
            {
                DisplayRichTextboxContentAndScroll("返回的字节数组的长度范围为 1~250");
                return false;
            }
            if (startAddress < 0 || startAddress > 65535)
            {
            ...
            {
                DisplayRichTextboxContentAndScroll("解析接收数据非法，接收的实际数据长度【不是】读取寄存器数量的2倍");
                return false;
            }
        ...
        public bool WriteSingleHoldingRegisterValue<T>(int startAddress, T value)
        {
            ...
            {
                //暂不考虑 char(就是ushort，两个字节)，decimal（十六个字节）等类型
                DisplayRichTextboxContentAndScroll("写Modbus数据暂不支持其他类型：" + value.GetType());
                return false;
            }
            try
            {
        ...
        public bool WriteManyHoldingRegisterValue(int startAddress, byte[] buffer)
        {
            ...
            {
                DisplayRichTextboxContentAndScroll("写连续寄存器块范围：(1 至120 个寄存器)");//每个寄存器将数据分成两字节
                return false;
            }
            byte[] addrArray = BitConverter.GetBytes((ushort)startAddress);
            //需要写入的寄存器个数
        ...
        public void DisplayBuffer(byte[] buffer, int count, bool isSend)
        {
            ...
            DisplayRichTextboxContentAndScroll(content);
        }
        #endregion
        ...
        public void DisplayRichTextboxContentAndScroll(object addContent)
        {
            try
            {
                uc.Invoke(new MethodInvoker(() =>
                {
                    uc.rtxtDisplay.AppendText(addContent + "\n");
                    uc.rtxtDisplay.ScrollToCaret();
                }));
            }
            catch (Exception)
            {
