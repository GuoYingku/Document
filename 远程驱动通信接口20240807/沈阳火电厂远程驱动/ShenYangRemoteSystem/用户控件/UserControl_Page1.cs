...
namespace ShenYangRemoteSystem.用户控件
{
    public partial class UserControl_Page1 : UserControl
    {
        ...
        private void UserControl_Page1_Load(object sender, EventArgs e)
        {
            Thread thread1 = new Thread(new ThreadStart(Uc1LifeProcess));
        ...
        //图片更新进程
        private void Uc1LifeProcess()
        {
            while (true)
            {
                //此处没有使用局部变量表示的全局变量类对象，而是使用了ControlHelper类中实例化的全局变量类对象，这里先观察以下可不可行，如果可行的话以后都用这种方案。
                //SystemVariables systemVariables = Form1.systemVariables;
